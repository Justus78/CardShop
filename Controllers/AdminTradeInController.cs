...
namespace api.Controllers
{
    ...
    public class AdminTradeInController : ControllerBase
    {
        ...
        [HttpPatch("trade-in/update")]
        public async Task<IActionResult> UpdateTradeStatus([FromBody] UpdateTradeInStatusDto dto)
        {
            var tradeIn = await _adminTradeInService.UpdateTradeInStatusAsync(dto);
            return tradeIn ? Ok(tradeIn) : BadRequest(tradeIn);
        }
    }
}
